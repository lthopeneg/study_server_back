...
{project_file_rule}'''
    ...
    blank_answer_rule = (
        '- 이 주제에서는 단일 식별자 외에 안전한 고정 포맷을 나타내는 한 개의 문자열 리터럴 또는 '
        'CultureInfo.InvariantCulture와 같은 단순 멤버 접근 하나를 정답으로 사용할 수 있습니다. '
        '함수 호출 전체나 여러 연산이 결합된 표현식은 정답으로 만들지 않습니다.'
        if '포맷 스트링' in minor_topic
        else '- 각 빈칸의 정답은 함수명, 메서드명, 변수명, 상수 같은 단일 식별자 하나가 되도록 주변 코드를 구성합니다.'
    )
...
- 포맷 스트링 삽입 주제는 단일 식별자 외에 안전한 고정 포맷 문자열 리터럴 또는 단순 멤버 접근 하나를 정답으로 사용할 수 있지만, 함수 호출 전체나 복합 표현식은 사용하지 않습니다.
